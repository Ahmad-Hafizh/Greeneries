...
export function SignupForm({ className, ...props }: React.ComponentProps<'form'>) {
  ...
  const onHandleSubmit = async (values: z.infer<typeof signupSchema>) => {
    setErrorMessage('');
    setLoading(true);
    try {
      const response = await callApi.post('/auth/signup/credentials', {
        name: values.name,
        email: values.email,
      });
    ...
    <Form {...form}>
      <form className={cn('flex flex-col gap-6', className)} {...props} onSubmit={form.handleSubmit(onHandleSubmit)}>
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-muted-foreground text-sm text-balance">sign up using your email and name</p>
        </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="grid gap-3">
                ...
                <FormControl>
                  <Input id="name" type="text" placeholder="username" required {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
